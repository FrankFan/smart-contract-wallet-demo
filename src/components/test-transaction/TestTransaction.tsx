...
export const TestTransaction = ({
  ...
  // 根据 scw余额 和 prefund 进行判断
  const preFund = useMemo(() => {
    const preFund = minFundsEstimate.sub(balance)
    console.log('balance ==== ', balance.toNumber())
    console.log('minFundsEstimate ==== ', minFundsEstimate.toNumber())
    console.log('preFund ==== ', preFund.toNumber())

    return preFund.gte(0) ? preFund : BigNumber.from(0)
    // return BigNumber.from(0.0009)
    // return BigNumber.from(FixedNumber.from(0.0009))
